<template>
    ...
    <div class="pageContent">    <!-- :row-key="record=>record.id" -->
      ...
      <!-- 再用一个变量判断是查看还是编辑，添加感觉和编辑差不多，编辑和添加用input框住，
       ...
       >  
              ...
              <span v-if="typeOperation==='check'">{{editForm.activityDescription}}</span> <!-- 这个editForm初始值，还得赋值为那一行的数据吧 -->
                        <el-input v-if="typeOperation==='edit'" type="textarea" rows="15" v-model="editForm.activityDescription">{{editForm.activityDescription}}</el-input>
              <el-input v-if="typeOperation==='add'" type="textarea" rows="15" v-model="editForm.activityDescription">{{editForm.activityDescription}}</el-input>
                    </el-form-item>
    ...
    const rulesEditForm = reactive({   /* 定义校验规则 */
      ...
      activityDescription:[{required:true,message:'请输入实践的内容！',trigger:'blur'}]
    })
    ...
    const loadactivityTable = async() =>{   //查找所有的数据,这个接口是不是有点问题,学生端还用传userNumber吗
      await service.post('/api/activity/find',{token:localStorage.getItem("token")}).then(res => {
        if (res.data.success) {
          const data = res.data;
