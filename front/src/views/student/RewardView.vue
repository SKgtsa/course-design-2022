<template>
    ...
    <div class="pageContent">    <!-- :row-key="record=>record.id" -->
      ...
      > <!-- 显示斑马纹和边框 -->
        ...
        <el-table-column label="标题" prop="rewardName" width="350" show-overflow-tooltip />
      ...
      <!-- 再用一个变量判断是查看还是编辑，添加感觉和编辑差不多，编辑和添加用input框住，
       ...
       >  
                ...
                v-model = "editForm.rewardDate" 
              />
                    </el-form-item> -->
                    <el-form-item label="标题" prop="rewardName">
              <span v-if="typeOperation==='check'">{{editForm.rewardName}}</span> <!-- 这个editForm初始值，还得赋值为那一行的数据吧 -->
                        <el-input v-if="typeOperation==='edit'" v-model="editForm.rewardName">{{editForm.rewardName}}</el-input>
              <el-input v-if="typeOperation==='add'" v-model="editForm.rewardName"></el-input>
                    </el-form-item>
                    <el-form-item label="内容" prop="rewardDescription">
              <span v-if="typeOperation==='check'">{{editForm.rewardDescription}}</span> <!-- 这个editForm初始值，还得赋值为那一行的数据吧 -->
                        <el-input v-if="typeOperation==='edit'" type="textarea" rows="15" v-model="editForm.rewardDescription">{{editForm.rewardDescription}}</el-input>
              <el-input v-if="typeOperation==='add'" type="textarea" rows="15" v-model="editForm.rewardDescription">{{editForm.rewardDescription}}</el-input>
                    </el-form-item>
    ...
    const rewardTableData = computed(() =>
      tableData.filter(
        (data) =>
          !search.value ||
          data.rewardName.toLowerCase().includes(search.value.toLowerCase())
      )
    )
...
const loadrewardTable = () =>{   //查找所有的数据,这个接口是不是有点问题,学生端还用传userNumber吗
  ...
  .catch(function(error) {
    ...
    const handleCheck = (row)=>{   //查看单个的数据 //一条一条赋值吧，一起赋值出bug了
      ...
      editForm.rewardId = row.rewardId;
    ...
    const handleDelete= (row)=>{  //删  //异步可能有问题
      ElMessageBox.confirm(
        ...
        .then(() => {
          service.post('/api/reward/delete',{token:localStorage.getItem("token") ,id:row.id}).then(res=>{
            if(res.data.success){
              messageSuccess('删除成功!')
    ...
    const sumbitEditRow=()=>{
      ...
      if(typeOperation.value==='check'){
        ...
        {token:editForm.token,rewardName:editForm.rewardName,rewardDescription:editForm.rewardDescription,id:editForm.rewardId})
        ...
        service.post('/api/reward/save',
        {token:editForm.token,rewardName:editForm.rewardName,rewardDescription:editForm.rewardDescription})
        .then(res=>{
          if(res.data.success){
            messageSuccess("添加成功！")
            typeOperation.value='';
