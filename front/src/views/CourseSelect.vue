...
const checkCourseTime = () => {   //更新课表页
    ...
    for (let i = 0; i < backData.length; i++) {
        for (let j = 0; j < backData[i].classTime.section.length; j++) {
            let n = backData[i].classTime.section[j] - 1; //数组下标从0开始
            if (backData[i].classTime.weekDay[j] == 1) {
                console.log(backData[i].courseName);
                tableData[n].Monday = '课程名称:' + backData[i].courseName + "  " + '@上课地址:' + backData[i].location;
            } else if (backData[i].classTime.weekDay[j] == 2) {
                tableData[n].Tuesday = '课程名称:' + backData[i].courseName + "  " + '@上课地址:' + backData[i].location;
            } else if (backData[i].classTime.weekDay[j] == 3) {
                tableData[n].Wednesday = '课程名称:' + backData[i].courseName + "  " + '@上课地址:' + backData[i].location;
            } else if (backData[i].classTime.weekDay[j] == 4) {
                tableData[n].Thursday = '课程名称:' + backData[i].courseName + "  " + '@上课地址:' + backData[i].location;
            } else if (backData[i].classTime.weekDay[j] == 5) {
                tableData[n].Friday = '课程名称:' + backData[i].courseName + "  " + '@上课地址:' + backData[i].location;
            } else if (backData[i].classTime.weekDay[j] == 6) {
                tableData[n].Saturday = '课程名称:' + backData[i].courseName + "  " + '@上课地址:' + backData[i].location;
            } else if (backData[i].classTime.weekDay[j] == 7) {
                tableData[n].Sunday = '课程名称:' + backData[i].courseName + "  " + '@上课地址:' + backData[i].location;
            }
        }
    }
}
...
//交互的时候给它解开
//***************************************************************
/* let courseInfo = ref([]); //后端给我的所有课程，用于选课
let backData = ref([]); //我的课程，用于生成课表
let tableData = reactive([ //课表中的数据
    {
        title: '第一大节(8:00-9:50)',
        Monday: '',
...
//发送请求后传给我一个数组，数组中有每个课程，我绑定到courseInfo中
    await service.post('/api/course/',).then(res=>{
        let data = res.data;
        if(data.success){
...
courseSelected();
 ...
 const dropCourse = (row) => { //学生的id没有写
    ...
    .then(() => { //学生的Id没有填
      service.post('/api/course/quit', { token: localStorage.getItem("token"), courseId: row.id,studentId: }).then(res => {
        if (res.data.success) {
          messageSuccess('退课成功!')
...
const handleSelect =async(row) => { //选课
    await service.post('/api/course/select',).then(res=>{
        let data = res.data;
        if(data.success){
            messageSuccess('选课成功！');
            localStorage.setItem('token',data.token);
